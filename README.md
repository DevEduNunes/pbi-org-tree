...
- **Card color from data**: a column with a color such as `#64C8C8`; text contrast is adjusted automatically
- **Tooltip text** and **link** (opens in the browser) per employee
- **Connectors**: elbow (rounded), curve or straight, custom width/color, optional dot at the child end
- Format pane: layout, card size / radius / border, avatar, text sizes, colors, connectors
...
| Card color | no | A CSS color such as `#64C8C8` |
| Tooltip text | no | Shown on hover instead of the name |
| Link URL | no | `https://` link, opened from a small arrow on the card |
